...
// select audio core
#if !defined(SR_USE_MEGA) && !defined(SR_USE_TINY)
#define SR_USE_STANDARD
#endif
...
#if defined(SR_USE_MEGA)
constexpr int BLOCK_SIZE = 256;                  // I2S buffer size (samples)
#else
...
static float fftAddAvg(int from, int to);                                      // average of FFT bins
...
#ifdef SR_USE_MEGA
#define NUM_GEQ_CHANNELS 32                                           // number of frequency channels. Don't change !!
#define NUM_PINK_CHANNELS 16                                          // number of "channels" in Pink Noise table. will be scaled up to 32 on-the-fly
#define NUM_SYNC_CHANNELS 16                                          // number of "channels" for UDP sound sync. Inly 16 supported currently
#else
#define NUM_GEQ_CHANNELS 16                                           // number of frequency channels. Don't change !!
#define NUM_PINK_CHANNELS 16                                          // number if "channels" in Pink Noise table.
#define NUM_SYNC_CHANNELS 16                                          // number of "channels" for UDP sound sync. Inly 16 supported currently
#endif
...
static const float fftResultPink[MAX_PINK+1][NUM_PINK_CHANNELS] = { 
  ...
  private:
    ...
    */
    ...
    void transmitAudioData()
    {
      ...
      for (int i = 0; i < NUM_SYNC_CHANNELS; i++) {
    ...
    void decodeAudioData(int packetSize, uint8_t *fftBuff) {
      ...
      for (int i = 0; i < NUM_SYNC_CHANNELS; i++) fftResult[i] = receivedPacket->fftResult[i];
